---
title: 'Threatening Communication: Diffusing the Scientific Evidence'
...
basePathVector <- c("B:/Data/research/fear is a bad counsellor/intervention/threatening-communication-diffusing-the-evidence",
                    "");
...
workingPath <- file.path(basePath, "results - output");
...
setFigCapNumbering(resetCounterTo=2);
setTabCapNumbering();
...
dat$role <- factor(dat$role,
                   labels=c("Intervention developers",
...
dat$employer <- factor(ifelse(is.na(dat$employer),
                              rep(8, nrow(dat)),
                              convertToNumeric(dat$employer)),
                       labels=c("A national health promoting organisation",
...
########################################################################
### Dichotomize role variable
########################################################################

dat$role_dichotomous <- ifelse(as.character(dat$role) %in%
                                 c("Intervention developers",
                                   "Program/campaign leaders",
                                   "Scientists"),
                               1, 2);

dat$role_dichotomous <- factor(dat$role_dichotomous,
                               labels=c("Close",
                                        "Distant"));
...
```{r table1, tab.cap='Sample sizes at t0, t1 and t2 and sample characteristics.'}
########################################################################
...
```{r descriptives, tab.cap=c("Distribution of gender.", "Distribution of role.", "Distribution of employers.")}
...
```{r beliefs, fig.cap="Participants' beliefs regarding behavior change.", fig.width=18/2.54, fig.height=16/2.54 }
########################################################################
### Sort and visualise beliefs
########################################################################
...
fig2 <- duoComparisonDiamondPlot(dat,
                                 items=relevantStatements[sortOrder],
                                 compareBy='role_dichotomous',
                                 labels=statements[sortOrder],
                                 comparisonColors=viridis(2, end=.8),
                                 conf.level=c(.99, .95),
                                 dataSize=2, dataAlpha=.33,
                                 jitterWidth=.4, jitterHeight=.4)

ggsave(fig2,
       file=file.path(workingPath, 'figure 2.png'),
       width=22, height=22, units='cm', dpi=300,
       type='cairo');

### (reverting sort order to match ggplot)

pander(varsToDiamondPlotDf(dat,
                           items=relevantStatements[rev(sortOrder)],
                           labels=statements[rev(sortOrder)],
                           conf.level=.99)[, 1:4]);

pander(associationMatrix(dat,
                         relevantStatements[rev(sortOrder)],
                         'role_dichotomous',
                         conf.level=.95));
...
```{r indices, fig.cap="Participants' belief indices regarding behavior change.", tab.cap="Associations between the belief indices and proximity to behavior change science.", fig.width=18/2.54, fig.height=16/2.54 }
########################################################################
### Compute indices
########################################################################
...
indicesSortOrder <- order(colMeans(dat[, names(scales)], na.rm=TRUE));

fig3 <- duoComparisonDiamondPlot(dat,
                                 items=names(scales)[indicesSortOrder],
                                 labels=c("It is mostly important that\ninterventions increase risk perception",
                                          "Threatening interventions should\nalways address efficacy",
                                          "Interventions should use\nemotion and confrontation",
                                          "Common sense is reliable in\nintervention development",
                                          "Increasing severity can cause\nsusceptibility to increase"),
                                 compareBy='role_dichotomous',
                                 comparisonColors=rev(viridis(2, end=.8)),
                                 conf.level=c(.99, .95),
                                 dataSize=2, dataAlpha=.33,
                                 jitterWidth=.4, jitterHeight=.4,
                                 xbreaks=1:5);
...
ggsave(fig3,
       file=file.path(workingPath, 'figure 3.png'),
...
pander(varsToDiamondPlotDf(dat,
                           items=relevantStatements[rev(indicesSortOrder)],
                           labels=statements[rev(indicesSortOrder)],
                           conf.level=.99)[, 1:4]);

pander(associationMatrix(dat,
                         names(scales)[rev(indicesSortOrder)],
                        'role_dichotomous',
                        conf.level=.95));
...
dat$interventionTime <- sapply(strsplit(dat$totalGroupTime, ":"),
                               function(x) {
                                 if (length(x)==1) {
                                   return(NA);
                                  } else {
...
## Time spent on the intervention page

Participants spent on average `r floor(mean(dat$interventionTime, na.rm=TRUE) / 60); ` minutes and `r round(((mean(dat$interventionTime, na.rm=TRUE) / 60) %% 1) * 60, 2); ` seconds on the page with the intervention.

```{r, results='asis'}

pandoc.header('Descriptives of whether participants considered the provided intervention clear.', level=2);

pander(descr(dat$clearinfo_clear));

pandoc.header('Descriptives of whether participants indicated that they were convinced by the intervention.', level=2);

pander(descr(dat$convinced_conv));

```

# Did intervention evaluation of fear appeal interventions differ?

```{r fig.width=10, fig.height=3}

t0_fear_variables <- grep('t0.*fear.*', names(dat), value=TRUE);

dat$t0_v1_fear_mean <- rowMeans(dat[, t0_fear_variables], na.rm=TRUE);

meansComparisonDiamondPlot(dat, items='t0_v1_fear_mean',
                           compareBy='role_dichotomous',
                           labels="",
                           dataAlpha=.5,
                           jitterWidth=0,
                           ...
                           xbreaks=c(1:10),
                           comparisonColors=rev(viridis(2, end=.8)),
                           outputFile = file.path(workingPath, 'figure 4.png'),
                           outputWidth=19,
                           outputHeight=5);

duoComparisonDiamondPlot(dat, items='t0_v1_fear_mean',
                         compareBy='role_dichotomous',
                         label="",
                         comparisonColors=rev(viridis(2, end=.8)),
                         dataAlpha=.5,
...
pander(meanDiff(x=dat$role_dichotomous, y=dat$t0_v1_fear_mean));

meansComparisonDiamondPlot(dat, items='t0_v1_fear_mean',
                           compareBy='role',
                           comparisonColors=viridis(7),
                           dataAlpha=.5,
                           jitterHeight=.1,
                           xbreaks=c(1:10));
...
moltenData <- suppressWarnings(reshape::melt(dat, id.vars=idVars));
...
```{r fig.width=10, fig.cap="The evaluation of the interventions of each type over time.", messages=FALSE}

########################################################################
########################################################################
### Visualisations of multilevel analyses
########################################################################
########################################################################

lDatExcerpt <- lDat[, c('interventionType', 'measurementMoment', 'value')];
lDatExcerpt <- lDatExcerpt[complete.cases(lDatExcerpt), ];

diamondLayerDat <-
  by(lDatExcerpt,
     lDatExcerpt$interventionType:lDatExcerpt$measurementMoment,
     function(Df) {
       ciVector <- meanConfInt(Df$value);
       ciVector <- c(ciVector$output$ci[1],
                     mean(Df$value, na.rm=TRUE),
                     ciVector$output$ci[2]);
       res<- diamondCoordinates(ciVector,
                                direction = "vertical",
                                fixedSize=.1);
       res$measurementMoment <- as.factor(rep(unique(Df$measurementMoment),
                                          nrow(res)));
       res$interventionType <- as.factor(rep(unique(Df$interventionType),
                                         nrow(res)));
       return(res);
     });

### Set x coordinate adjustments and colors
timeAdjustment <- c('t0' = 0,
                    't1' = 1,
                    't2' = 2);
typeAdjustment <- c('fear' = -.25,
                    'know' = 0,
                    'scd' = .25);

### Adjust x coordinates
diamondLayerDat <-
  lapply(names(diamondLayerDat),
         function(diamondName) {
           diamondLayerDat[[diamondName]]$x <-
             diamondLayerDat[[diamondName]]$x +
             timeAdjustment[which(sapply(names(timeAdjustment), grep, x = diamondName) == 1)];
           diamondLayerDat[[diamondName]]$x <-
             diamondLayerDat[[diamondName]]$x +
             typeAdjustment[which(sapply(names(typeAdjustment), grep, x = diamondName) == 1)];
           return(diamondLayerDat[[diamondName]]);
         });

### Generate diamonds
diamondLayers <-
  lapply(diamondLayerDat,
         geom_polygon,
         mapping = aes(x=x, y=y));

ggplot(lDat,
       aes_string(x = 'measurementMoment',
                  y = 'value',
                  color = 'interventionType',
                  fill = 'interventionType',
                  group = paste0('measurementMoment:interventionType'))) +
  geom_violin(alpha = .2,
              #color=NA,
  ...
  geom_point(alpha = .1,
             position = position_jitterdodge(dodge.width=.75,
                                             ...
                                             jitter.height=.3),
             na.rm=TRUE)+
  geom_line(aes_string(group='interventionType'),
  ...
  xlab("Measurement moment") +
  ylab("Intervention evaluation") +
  theme_minimal() +
  scale_fill_viridis(discrete=TRUE,
                     end=.8,
                     labels=c("Fear appeal", "Knowledge", "Social cognitive")) +
  scale_color_viridis(discrete=TRUE,
                     end=.8,
                     labels=c("Fear appeal", "Knowledge", "Social cognitive")) +
  guides(fill=guide_legend(title=NULL), color=guide_legend(title=NULL));

ggsave(file=file.path(workingPath, 'figure 5.png'),
...
### Show results
print(summary(valueLmer), width=120);

```

# Qualitative data

These are participants' responses to the question "Could you explain what you did not find convincing?" ("Kunt u uitleggen wat u niet overtuigend vond?"), which was asked to all participants who answered anything lower than the maximum score ("Completely", "Volledig") on the preceding question, "Did this information convince you?" ("Ben u door deze informatie overtuigd?").

```{r qualitative-results, results='asis'}
  cat(paste("* ", na.omit(dat$notconvinced_reason),
            collapse="\n"));
```
